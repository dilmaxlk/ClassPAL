...
$sql = "SELECT COUNT(sa_id) FROM cp_subj_allo";
...
$sql = "SELECT cp_subj_allo.sa_id, cp_subj_allo.sa_stu_student_id, cp_subj_allo.sa_subj_id, cp_subj_allo.sa_institutid, cp_subj_allo.sa_subj_fee, cp_subj_allo.sa_batch_no, cp_subj_allo.sa_notes, cp_subj_allo.sa_barCode, cp_students.stu_studentname, cp_subjects.subj_name FROM `cp_subj_allo` INNER JOIN `cp_students` ON cp_subj_allo.sa_stu_student_id=cp_students.stu_studentID INNER JOIN `cp_subjects` ON cp_subj_allo.sa_subj_id = cp_subjects.subj_id $limit";
$query = mysqli_query($db, $sql);
...
if($last != 1){
	...
	if ($pagenum > 1) {
        $previous = $pagenum - 1;
		$paginationCtrls .= '<li><a href="index.php?page=ViewAllAllowStudents&SubjectID='.$Subj_ID.'&PageNo=1">&laquo;&laquo;</a></li>'
                                 .'<li><a href="index.php?page=ViewAllAllowStudents&SubjectID='.$Subj_ID.'&PageNo='.$previous.'">&laquo;</a></li>';
                         
		// Render clickable number links that should appear on the left of the target page number
		for($i = $pagenum-2; $i < $pagenum; $i++){
			if($i > 0){
		        $paginationCtrls .= '<li><a href="index.php?page=ViewAllAllowStudents&SubjectID='.$Subj_ID.'&PageNo='.$i.'">'.$i.'</a></li>';
			}
	    }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<li><a href="index.php?page=ViewAllAllowStudents&SubjectID='.$Subj_ID.'&PageNo='.$next.'">&raquo;</a></li> '
                         .'<li><a href="index.php?page=ViewAllAllowStudents&SubjectID='.$Subj_ID.'&PageNo='.$last.'">&raquo;&raquo;</a></li>'
                         .'</ul>';
    }
}
...
?>
        ...
        <div class="box-body">
         ...
         <!-- Search Form -->       
                ...
                <form style="margin-bottom: 10px;" role="form" method="get" action="" class="form-inline">
                    <input type="hidden" name="page" value="ViewAllAllowStudents">
                    ...
                    <a style="margin-top: 10px;" href="index.php?page=ViewAllAllowStudents&SubjectID=<?php echo $Subj_ID; ?>&PageNo=1" class="btn btn-success btn-flat" >View All</a>
 ...
 ?>
            <?php
               $stmt3 = $db->prepare("SELECT COUNT(sa_stu_student_id) FROM cp_subj_allo");
      ...
      <h4 class="box-title">Total Students: <?php echo $TotalStudents; ?></h4>
                    ...
                    <tbody>
                      ...
                      <tr>
                         ...
                         <td>
                             <?php echo htmlentities($row['stu_studentname'], ENT_QUOTES, "UTF-8"); ?>
                         ...
                         <!-- <td><a href="../output/barcode/ex.php?StudentID=<?php //echo $row['sa_stu_student_id']; ?>&StudentName=<?php //echo $row['stu_studentname']; ?>" target="_blank" class="btn btn-success btn-flat" >Generate</a></td>  -->   
                          ...
                          <td>
                                <a style="margin-top: 10px;" href="index.php?page=AddPayment&StudentID=<?php echo htmlentities($row['sa_stu_student_id'], ENT_QUOTES, "UTF-8"); ?>&StudentName=<?php echo htmlentities($row['stu_studentname'], ENT_QUOTES, "UTF-8"); ?>&SubjID=<?php echo $row['sa_subj_id']; ?>&SubjName=<?php echo $row['subj_name']; ?>&ReceiptNo=<?php echo $AutoNuber;   ?>&BatchID=<?php echo $row['sa_batch_no']; ?>" target="_blank" class="btn btn-success btn-flat" >Add Payment</a>
                                <a style="margin-top: 10px;" href="index.php?page=StudentAttendance&StudentID=<?php echo htmlentities($row['sa_barCode'], ENT_QUOTES, "UTF-8"); ?>" target="_blank" class="btn btn-info btn-flat" >Mark Attendance</a>
			...
			<SCRIPT LANGUAGE="JavaScript">
                                    ...
                                    swal({   title: "Are you sure?",   text: "Do you want to delete these Students ?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes, delete them all!",   cancelButtonText: "No, No.. Cancel Please!",   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform.action = "actions/deleteallstuallo.php?page=ViewAllAllowStudents&SubjectID&PageNo=<?php echo $PNo ?>";  document.myform.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error");   } });
 ...
 <?php
                       ...
                       $sql_2 = "SELECT cp_subj_allo.sa_id, cp_subj_allo.sa_stu_student_id, cp_subj_allo.sa_subj_id, cp_subj_allo.sa_institutid, cp_subj_allo.sa_subj_fee, cp_subj_allo.sa_batch_no, cp_subj_allo.sa_notes, cp_subj_allo.sa_barCode, cp_students.stu_studentname, cp_subjects.subj_name FROM `cp_subj_allo` INNER JOIN `cp_students` ON cp_subj_allo.sa_stu_student_id=cp_students.stu_studentID INNER JOIN `cp_subjects` ON cp_subj_allo.sa_subj_id = cp_subjects.subj_id WHERE (cp_subj_allo.sa_stu_student_id LIKE '%{$SearchKey}%' OR cp_students.stu_studentname LIKE '%{$SearchKey}%' OR cp_subj_allo.sa_batch_no LIKE '%{$SearchKey}%' OR cp_subj_allo.sa_barCode LIKE '%{$SearchKey}%')";
            ...
            <?php
               $stmt4 = $db->prepare("SELECT COUNT(sa_stu_student_id) FROM cp_subj_allo WHERE sa_batch_no LIKE '%{$SearchKey}%'");
               ...
               $stmtInsName = $db->prepare("SELECT COUNT(sa_stu_student_id) FROM cp_subj_allo INNER JOIN `cp_ins` ON cp_ins.ins_id = cp_subj_allo.sa_institutid WHERE cp_ins.ins_name LIKE '%{$SearchKey}%'");
    ...
    <table id="vas_table2" class="table table-hover table-bordered table-responsive">
                    ...
                    <tbody>
                      ...
                      <tr>
                         ...
                         <?php
                                $a2 = mt_rand(100000,1000000); 
                                
                                $AutoNuber2 = $a2 + 159357;
                          ?>
                         
                         
                         <td>
                                <a style="margin-top: 10px;" href="index.php?page=AddPayment&StudentID=<?php echo htmlentities($row['sa_stu_student_id'], ENT_QUOTES, "UTF-8"); ?>&StudentName=<?php echo htmlentities($row['stu_studentname'], ENT_QUOTES, "UTF-8"); ?>&SubjID=<?php echo $row['sa_subj_id']; ?>&SubjName=<?php echo $row['subj_name']; ?>&ReceiptNo=<?php echo $AutoNuber2;   ?>&BatchID=<?php echo $row['sa_batch_no']; ?>" target="_blank" class="btn btn-success btn-flat" >Add Payment</a>
                                <a style="margin-top: 10px;" href="index.php?page=StudentAttendance&StudentID=<?php echo htmlentities($row['sa_barCode'], ENT_QUOTES, "UTF-8"); ?>" target="_blank" class="btn btn-info btn-flat" >Mark Attendance</a>
			...
			<SCRIPT LANGUAGE="JavaScript">
                                    ...
                                    swal({   title: "Are you sure?",   text: "Do you want to delete ?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes, delete them all!",   cancelButtonText: "No, No.. Cancel Please!",   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform2.action = "actions/deletestuallo.php?page=ViewSubjectAllocatedStudents&SubjectID=<?php echo $SUBJECT_ID2; ?>&SearchKey=<?php echo $SKey; ?>";  document.myform2.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error");   } });
