...
<html>
  ...
  <body onload="window.print();">
        ...
        <div class="row">
          <div class="col-xs-12">
            <h2 class="page-header">
              <i class="fa fa-usd"></i> Report: Daily Income | Course Name: <?php echo $AS_Sunj_Name; ?> | Date: <?php echo $date; ?>
    ...
    $stmt = $db->prepare("SELECT Pay_stu_studentID, pay_student_name, pay_subj_id, pay_paymentdate, pay_paymentmonth, pay_cos_fee, pay_cos_admi, pay_cos_total, pay_misc_pay_description, pay_misc_amount FROM `cp_payments` WHERE pay_paymentdate LIKE '%{$date}%' AND pay_subj_id = $Subj_ID ORDER BY pay_paymentdate DESC");
        ...
        <div class="row">      
          <div class="col-xs-6">
            ...
            ?>
                    ...
                    <th>Total Course Income: <?php echo $TotalIncome; ?></th>
            <?php
                // Get total Mis. income
               $stmt_Mise_income = $db->prepare("SELECT SUM(pay_misc_amount) FROM cp_payments WHERE pay_paymentdate LIKE '%{$date}%' AND pay_subj_id = $Subj_ID");
