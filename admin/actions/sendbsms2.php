...
include_once '../php-includes/connect.inc.php'; 
     ...
     if (isset($_GET["SMSKey"])){
          ...
          $To = $_GET["TO"];
                ...
                $sql = "SELECT cp_exams.id, cp_exams.ex_studentid, cp_exams.ex_studentname, cp_exams.ex_subjid, cp_exams.ex_batchno, cp_exams.ex_examdate, cp_exams.ex_code, cp_exams.ex_marks, cp_exams.ex_grade, cp_exams.act_b_rank, cp_exams_list.ex_code, cp_exams_list.ex_des, cp_students.stu_con_mobile1  FROM `cp_exams` INNER JOIN `cp_students` ON cp_students.stu_studentID = cp_exams.ex_studentid LEFT JOIN `cp_exams_list` ON cp_exams_list.ex_code = cp_exams.ex_code WHERE cp_exams.ex_code LIKE '%{$ExCode}' AND cp_exams.ex_batchno LIKE '%{$SMSKey}' ORDER BY cp_exams.ex_marks DESC LIMIT $From,$To";
    ...
    $stmt_select_sms_gway_settings->execute();
        ...
        while ($stmt_select_sms_gway_settings->fetch()){
            ...
            if ($sms_gway_name === "SemySMS"){
                                    ...
                                    $msg = "StudentName:{$row['ex_studentname']} ExamDate:{$row['ex_examdate']} ExamName:{$row['ex_des']} ExamMarks: {$row['ex_marks']} BatchRank: {$row['act_b_rank']} Grade: {$row['ex_grade']}"; // Message
                ...
                $text = urlencode("StudentName:{$row['ex_studentname']} ExamDate:{$row['ex_examdate']} ExamName:{$row['ex_des']} ExamMarks: {$row['ex_marks']} BatchRank: {$row['act_b_rank']} Grade: {$row['ex_grade']}");
